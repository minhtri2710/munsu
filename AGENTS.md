...
- Wake delivery (`internal/wakedelivery/`): Deep module consolidating report/relay/wake/notification. Two public functions: `DeliverWake` (soldier-side: status→receipt→event→wake) and `ReconcilePending` (captain-side: pending receipt→general relay→ack→obligation close). The CLI `report` command calls `DeliverWake`; `captain/relay.go` delegates to `ReconcilePending`. Pane injection lives here as `InjectToParentPane`/`InjectToParentPaneWithResolver`. Storage primitives stay in `turnend` (receipts, obligations) and `lifecycle` (wake queue, locks).
