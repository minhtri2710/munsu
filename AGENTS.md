...
- Delivery acceptance rules have one owner in `internal/domain/domain.go` (`PR.CanMerge`, `Review.IsApproving`); provider orchestration and CLI-compatible delivery operations live in `internal/fleet/delivery_*.go`.
- AFK supervision lives in `internal/orchestrator/afk_*.go`; focused tests stay beside those files. Durable task/meta mechanics used by it remain in `internal/home`.
