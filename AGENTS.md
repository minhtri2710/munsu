...
- AFK daemon (`internal/afk/`): `Daemon.Start` runs foreground (lock→flag→triage→signal→cleanup); `lock.go` identity lock; `sentinel.go` inject mark; `triage.go` `OneCycle` produces `Digest` from wake queue. CLI: `munsu afk start`.
## Go version
