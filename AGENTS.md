...
- Task lifecycle, dispatch, binding and delivery-invariant authority lives in `internal/taskauthority` (named operations fenced by Task Generation inside a transactional `Store.Update`); `internal/taskauthorityfs` implements the filesystem Store adapter, reconciles `.meta`/`.status` projections, and owns `munsu migrate task-authority` (plan/apply). `internal/home` no longer owns task lifecycle, dispatch or binding authority.
- AFK supervision lives in `internal/orchestrator/afk_*.go`; focused tests stay beside those files. Task state and `.meta` projections live in `internal/taskauthority` / `internal/taskauthorityfs`; `internal/home` retains generic storage mechanics (writer identity, leases, mailbox).
