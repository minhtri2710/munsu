...
## Deprecated flat command aliases
Eight hidden top-level aliases (`fleet-sync`, `fleet-snapshot`, `fleet-view`,
`bearings`, `review-diff`, `pr-check`, `pr-merge`, `merge-local`) are kept for
one more release. They are wired through check.sh scripts generated by
`internal/delivery/prcheck.go` — in-flight scripts reference the old flat form.

Next release: remove them from `root.go` registration and update all references.
The generated check.sh now emits `fleet sync` (non-deprecated).
## Module map
