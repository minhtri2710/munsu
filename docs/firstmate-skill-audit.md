...
| AFK mode | captain (human) | `/afk` or state/.afk or FM_INJECT_MARK | `afk` (`.agents/skills/afk/`) | AGENTS.md sec 9, line 346: "Invoke the /afk skill when the captain says /afk" | Loaded on `/afk` command or state detection | Optional | Daemon not started | `internal/orchestrator/afk_*.go` Daemon (`munsu afk`, `afk drain`, `afk return`) — diagnosis and manual reconciliation only; no nudge-injecting daemon (ADR-0013) |
