...
The shim is an operational safeguard against accidental primary-checkout, wrong-worktree, and casual force/delete mutation — not a security sandbox. Because it is a real `git` on PATH, the guard receives the same argv any invocation would after the shell has word-split it, so a Git invocation wrapped in another program (`sh -c "git push --force …"` or a script — anything the shell resolves to `git` through PATH) is now fenced too; that residual, open when the fence was a harness hook, is closed. Residuals that remain: an unlisted mutating verb (for example `stash`, `config`, `update-ref`, `symbolic-ref`) is outside the recognized set and passes; a Git-native alias (`git -c alias.x='push --force' x`, or a configured `alias.x`) is read by the guard as the verb token `x`, which is likewise outside the recognized set and passes — only a shell-level alias resolving `git` through PATH is fenced, never Git's own alias expansion; a Git invoked by absolute path (`/usr/bin/git`) never consults PATH and bypasses the shim; and an agent that resets its own PATH removes the shim directory. Closing those classes is out of scope for the ratified allowlist and would require argv interception below PATH resolution. The default delivery path is unaffected: reparenting the no-mistakes gate's Git daemon to PPID 1 is orphan adoption and says nothing about its PATH — the daemon runs with whatever environment its first spawning client had. Today that client is a login shell whose PATH carries no shim directory, so gate pushes never reach the fence (verified at merge). The latent fragility is a cold start from inside a shimmed soldier/captain session: the daemon then inherits `state/shim/bin`, and its gate pushes fail closed either way — with no task binding the fence refuses outright, and with an inherited binding it refuses because the gate worktree is not the bound worktree — until the daemon is restarted from an unshimmed shell.
