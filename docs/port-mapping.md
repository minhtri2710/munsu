...
| Endpoint observation (BEO-16/P1a) | typed orthogonal probe | `internal/backend` | **implemented**: `Lifecycle`/`Responsiveness`/`Freshness`/`Activity`/`Source`/`Incarnation`/`Detail`; `Backend.Alive` fully removed — every adapter exposes a structured `CheckAlive`/`CheckAgentAlive`. Adapters never fabricate freshness; Fleet concludes current vs stale against the exact canonical binding/incarnation (no dispose/relaunch on ambiguous). Native busy/event transport is P1b (Herdr `proposed`, not claimed current). |
