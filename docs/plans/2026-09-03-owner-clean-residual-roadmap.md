...
Still open: T2·C1 `captain config`, T2·C2 consolidation, gate G3 (ADR-0005 §6 Git fencing), ADR-0005 §5 General→Captain relay, and the ADR-0008 §2 `.meta`/`.status` projection-retention owner decision. (G1/G2 and T3 were resolved by ADR-0023 — see the status line above.)
...
T0 (G5 resolved) and T1 (R1/R2/R4 landed; R3 fully landed — PR-meta aliasing + `DeliveryState`/`MetaDeliveryState` consolidation — via commit e954c4d) landed 2026-09-03. G4 shipped (#744); G1/G2 and the T3 recovery stream (S1–S3) are retired by ADR-0023. The remaining work is small and independent: the T2·C1 `captain config` half, T2·C2, ADR-0005 §5 relay (T3·S4), gate G3 (ADR-0005 §6 Git fencing, security-adjacent), and the ADR-0008 §2 projection-retention owner call.

## Human gates

Every push, PR mutation, merge, and deploy. R2 and G3 additionally require a design decision recorded before implementation (coordinated contract replacement / security-adjacent). T3's design decision was resolved as won't-build (ADR-0023).
