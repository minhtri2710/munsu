...
Task 8.3 is complete through commit `0e9a274e` (docs-only: AGENTS.md, docs/architecture.md, docs/port-mapping.md, ADR-0007; +54/−18). Independent Reviewer acceptance verified all four criteria: `docs/architecture.md` describes `taskauthority` and `taskauthorityfs` accurately — every claim in the "Task state" and "Task Authority" sections spot-checked against the code at HEAD (the Aggregate/Generation/Revision/dispatch holds+decisions/typed audit/idempotency receipts owned by `internal/taskauthority` with all named ops present; the generation fence inside one `Store.Update` under `withDispatchLock` with idempotent Operation ID + digest replay and changed-digest `ErrOperationConflict`; `.meta`/`.status` reconciled projections per ADR-0007 §7 via `ReconcileProjections`/`ReconcileTaskProjections`; zero filesystem imports in `internal/taskauthority`; `.dispatch.lock`→per-task locking; recoverable write-ahead transactions with fault-stage journal; `munsu migrate task-authority` plan/apply; v1 homes fail closed `ErrMigrationRequired` across ready/consume-ready/report/brief/task show/git-safety/snapshot/observe; raw-`.meta`/backlog-only tasks present an empty authority view); `docs/port-mapping.md` maps canonical task lifecycle and dispatch ownership to the new module — all 7 new rows (lifecycle, meta+status projections, dispatch control, binding+ConfirmSpawn, delivery invariants, handoff receipt, migration) plus the corrected Promote row and the PR-check/merge rows verified against the code (`munsu spawn promote` → `Authority.Promote` at `cli/spawn_cmd.go:304/340`; `munsu captain handoff` → `destinationAuth.ReceiveTransfer` at `fleet/task_handoff_transaction.go:962`; `munsu migrate task-aggregates` absent), with the remaining `internal/home` references (projections primitives, durable mailbox) accurate as generic storage; `AGENTS.md` stale task/delivery ownership notes revised surgically — exactly three bullets changed (delivery bullet, one new module-map bullet with the explicit "`internal/home` no longer owns task lifecycle, dispatch or binding authority", AFK bullet re-pointed), whole-branch diff shows no intermediate commit touched AGENTS.md and all other sections verbatim; ADR-0007 status changes only if the implementation is complete — status changed to "Accepted; implementation complete" with all ADR §11 staged-break slices 1–8 independently verified landed (slice 8 = `internal/home` lifecycle/dispatch authority deleted at Tasks 6.2/8.1/8.2, at HEAD no `StartTask`/`UnblockTask`/`ReopenTask`/`BindTaskWorktree`/`BindTaskEndpoint`/`PromoteMeta`/`UpdateCurrentTaskAggregateState`/`CompareAndSwapMeta` in non-test home; the retained `dispatch_control.go` holds only decode shapes + the sanctioned Task 4.3 supervision), and the wording "implementation complete" (not "verified") is precise — Task 8.4 remains the verification gate. Adjudicated: the §5-vs-§3 citation lives only in the plan's Task 6.2/Checkpoint 6 records (the ADR itself cites ADR-0002/0004/0005 and revision ordering correctly in §3) — correctly flagged, not fixed; no doc file references the stale `task_lifecycle.go`/`dispatch_evaluator.go` legacy surface; the "Interact" row remains accurate (durable mailbox is generic home storage); the fail-closed command list is representative not exhaustive (e.g. `task add` also fails closed via `Store.Update`) — no correction required.

Task 8.4 is complete through the final gate at `b5264863`. Independent Reviewer acceptance (full final verification tier) verified all six criteria: architecture grep gates find no forbidden production call paths — the authority-mutation and home-lifecycle-mutation greps return ZERO production matches, and the raw-meta grep shows 11 `WriteMeta` sites fully classified (8 projection-layer writers with typed non-rollback errors, 2 `captain:` exemption sites, 1 runtime-only spawn side file) with zero `CompareAndSwapMeta`; both Store adapters pass contract tests and `-race` — the shared `storecontract.Run` suite executes in BOTH adapters (memstore + fs) with all contract behaviors covered (empty view, rollback on callback error, exactly-one revision advance, duplicate-op replay/conflict, generation replacement, multi-task rejection, holds/interpretations/decisions persistence, audit accumulation, digest/identity validation) and `-race` green on both; crash tests pass at every journal stage — `TestCrashRecoveryConvergesEveryStage` (10/10), `TestWorktreeBindingCrashRecoveryConvergesEveryStage` (10/10), `TestConfirmSpawnCrashRecoveryLeavesTaskQueuedOrWorking` (7/7), `TestMigrationApplyResumesAfterCrash` (4/4) plus the fleet/cli Crash|Journal|Recover integration suites; migration plan/apply and already-migrated paths pass — home (7) + taskauthorityfs (24) migration tests incl. read-only/deterministic plan, consumes-v1-aggregate-output, converts-all-record-families, quarantine, apply happy path, already-migrated no-rewrite, source/home-identity/target-conflict fail-closed, crash resume, symlink/hardening, plus the integration-tagged CLI `TestMigrateTaskAuthority*` plan/apply/already-migrated/quarantine suite; full build, vet, and tests pass with every remaining failure proven identical to the Phase-0 baseline — full unit + full integration suites exit 0 (13 packages) with only the sanctioned skip, a no-skip run proves `TestAgentSkillMirrorsMatchCanonical` is the ONLY failure anywhere (from the pre-branch chore commit `0bce4616`, unrelated to Task Authority), and the plan's focused package gate is green; no-mistakes equivalence is fully evidenced (lint = build+vet PASS; test = `go test ./...` PASS modulo the single sanctioned skip) — reported, not executed, per the no-push/PR constraint with the branch left clean for primary merge and the final unskipped verification at the primary checkout. Final-plan checks: ALL 185 plan boxes are now `[x]` (179 before this record + the six Task 8.4 criteria), the branch is clean with commit history ordered per the plan, and the completion-packet items are evidenced (accepted task/phase list, review agents + rulings, exact verification commands/results, allowed worktree skip, remaining risks, branch cleanliness). Optional cleanup items recorded (Lead decides, none blocking): the vestigial `LegacyTaskAuthoritySymbols` gate list remains (inert — the gate passes with an empty allowlist); the `munsu migrate task-aggregates` provenance comments remain in `taskauthorityfs/migration.go` (historically accurate, cosmetic); the Checkpoint 7 stale note is corrected via the Task 8.3 docs + record (the dated record wording stands as history); the pre-existing `internal/configmigration` gofmt drift remains byte-identical to the merge-base (no tidy applied). Observations: at completion, ADR-0004 status still read "Accepted; implementation pending" (unchanged since pre-branch; the Task 8.3 scope updated only ADR-0007); that status was later refreshed to "Accepted; substantially implemented" by the [owner-clean residual roadmap](../plans/2026-09-03-owner-clean-residual-roadmap.md) (2026-09-03), so this observation no longer holds. The primary checkout must still run the final unskipped verification per `primaryCheckoutMustRunUnskipped`.
...
- [x] `docs/architecture.md` describes `taskauthority` and `taskauthorityfs` accurately.
- [x] `docs/port-mapping.md` maps canonical task lifecycle and dispatch ownership to the new module.
- [x] `AGENTS.md` stale task/delivery ownership notes are revised without overwriting unrelated pre-existing edits.
- [x] ADR-0007 status changes only if the implementation is complete.
...
- [x] Architecture grep gates find no forbidden production call paths.
- [x] Both Store adapters pass contract tests and `-race`.
- [x] Crash tests pass at every journal stage.
- [x] Migration plan/apply and already-migrated paths pass.
- [x] Full build, vet, and tests pass or every remaining failure is proven identical to the Phase-0 baseline.
- [x] No-mistakes passes before push/PR.
