...
Munsu has strong parity in core fleet state, persistent watch operation, delivery pipelines, harness detection, self-update rebuilding, and basic second lifecycle commands. The remaining material gaps cluster around boundaries where a compiled fleet manager must integrate with external agent harnesses or reconstruct Git/delivery state after it has changed.
...
3. second launch passes a literal `$(cat .../AGENTS.md)` argument because `exec.Command` does not invoke a shell;
...
| Core second lifecycle surface | `internal/second/second.go`, `internal/cli/second_cmd.go` | Seed, launch, retire, handoff, list, and config-push verbs exist; several implementations need hardening below. |
...
### P0 — Second launch sends a literal shell expression instead of the charter

**Evidence**

`internal/second/second.go::buildLaunchArgs` appends:

```go
"$(cat "+filepath.Join(secondHome, "AGENTS.md")+")"
```
...
A second may start without its charter and therefore without its supervision boundaries. The same generic argv shape is also applied to every verified harness even though prompt/path semantics differ by adapter.

**Required behavior**

Read `AGENTS.md` in Go, pass its content through a harness-owned second launch template, and test exact argv for every supported harness. A harness without a verified noninteractive second launch contract must fail closed.

### P0 — Runtime no-mistakes gate containment is missing

**Evidence**

- Firstmate uses `bin/fm-gate-refuse-lib.sh` plus the shared primary-scope predicate in `bin/fm-primary-scope-lib.sh` to prevent gate-capable agents from acting as ordinary crews in the same checkout.
...
### P1 — Second registry, handoff, and inheritance do not match their stated contracts

**Evidence**

- `internal/second/second.go::List` scans `<parentHome>/seconds` despite its comment saying the full implementation reads `data/seconds.md`; it therefore omits registered homes outside that directory and loses scope/project/added metadata.
- `Seed` creates directories and `AGENTS.md` but no provenance marker equivalent to firstmate's `.fm-second-home`.
...
- Firstmate's `fm-backlog-handoff.sh` delegates complete block moves to `tasks-axi mv`, validates a seeded second home, and refuses cross-home dependency damage.

**Impact**

Registry output can be wrong, task ownership can split between homes, handoff can lose backlog semantics/dependencies, and seconds can drift from primary-authoritative captain preferences.

**Required behavior**

Make `data/seconds.md` the authoritative registry, introduce a safe seeded-home marker, delegate backlog moves to the configured backend, make multi-key moves atomic, and push `captain-shared.md` read-only with atomic replacement and strict destination validation. Add a locked live-second convergence sweep that validates homes, checks real agent liveness, propagates inherited material, and nudges only when the instruction surface changed.
...
### P2 — Live second convergence is not enforced at session start

**Evidence**

Firstmate `bin/fm-bootstrap.sh::second_sync` validates live second homes, fast-forwards them to the primary's local default-branch commit when safe, propagates inherited material, checks agent liveness, and sends a reread nudge only when the instruction surface changed. Munsu exposes individual second commands and status helpers, but session-start/bootstrap does not orchestrate an equivalent locked convergence sweep.

**Impact**

Persistent seconds can continue running stale instructions or inherited configuration while the primary reports a healthy session.
...
6. **"Second launch supports only Pi."** Obsolete as a nominal claim: the current registry accepts six harnesses. However, exact second argv semantics are not verified, and the literal prompt bug makes current operational support unsafe.

## Priority order

1. Establish and verify the separate development/runtime backlog contracts before any implementation work.
2. Fix second launch prompt correctness and fail-closed harness contracts.
...
8. Repair second registry/handoff/inheritance semantics.
