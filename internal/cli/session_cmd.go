...
func newWatchCmd() *cobra.Command {
	cmd := &cobra.Command{
		...
		RunE: withHome(func(cmd *cobra.Command, args []string, ctx Ctx) error {
			reason, err := orchestrator.RunWithProbeAndSender(ctx.Home, runtimeTaskEndpointProbe(), newSessionMailboxSender(), orchestrator.NewCaptainWatcherHooks(newSessionUplinkTransport(), newSessionActivationTransport()), fleetRetirementPort{}, runtimeTaskStatePort{})
			if err != nil {
				return err
			}
