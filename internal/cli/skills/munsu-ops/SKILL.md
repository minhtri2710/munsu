...
**Context pointer:** To update munsu itself or any secondmate home after the repo advances, run `munsu skill show munsu-update` and follow the auxiliary skill. It runs `munsu update` to fast-forward the install root, then the manual procedure for each secondmate home.
...
3. **Context** — content of `data/captain.md`, `data/learnings.md`, `data/projects.md`, and `data/secondmates.md` (first 20 lines each).
...
**Context pointer:** For secondmate lifecycle operations (seed, launch, retire, handoff, config-push), run `munsu skill show secondmate-provisioning` and follow the auxiliary skill.
