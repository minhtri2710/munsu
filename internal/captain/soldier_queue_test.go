...
func setupSoldierTestHomes(t *testing.T, agentStatus string) (captainHome, soldierTaskID, senderIdentity string) {
	...
	if err := mailbox.WriteHomeIdentity(captainHome, senderIdentity, mailbox.RankCaptain); err != nil {
...
func TestSendToSoldier_Idle_SendsNotificationRef(t *testing.T) {
	...
	if env.SenderRank != mailbox.RankCaptain {
...
func TestReconcileSoldierPending_WrongAckFailsClosed(t *testing.T) {
	...
	ack := &mailbox.ProcessingAck{
		...
		ReceiverRank:   env.ReceiverRank,
		ReceiverID:     env.ReceiverID,
		PayloadHash:    mailbox.PayloadHashHex("wrong payload"),
		ProcessedAt:    time.Now().UnixNano(),
		Outcome:        mailbox.OutcomeAccepted,
	}
	if err := store.WriteAck(ack); err != nil {
...
func TestSoldierReceiveNotification(t *testing.T) {
	...
	badRef := mailbox.NotificationRef{MessageID: "nonexistent", SenderIdentity: senderIdentity}
...
func TestSoldierAckNotification_WritesAck(t *testing.T) {
	...
	if ack.Outcome != mailbox.OutcomeAccepted {
		t.Errorf("outcome=%q, want %q", ack.Outcome, mailbox.OutcomeAccepted)
	}
	...
	if second.Outcome != mailbox.OutcomeAccepted {
