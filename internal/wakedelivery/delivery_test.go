...
func TestDeliverWake_SoldierMaterialState(t *testing.T) {
	...
	events, err := event.ReadAll(soldierHome)
	if err != nil {
	...
	receiptPath := turnend.ReceiptPath(captainHome, "test-soldier", "test-key")
	if _, err := os.Stat(receiptPath); err != nil {
		t.Errorf("captain receipt should exist: %v", err)
	}

	// Captain obligation should exist
	open, err := turnend.IsTaskReportRelayOpen(captainHome, "test-soldier")
	if err != nil {
...
func TestDeliverWake_SoldierNonMaterialState(t *testing.T) {
	...
	receiptPath := turnend.ReceiptPath(captainHome, "test-worker", "default")
	if _, err := os.Stat(receiptPath); err == nil {
...
func TestDeliverWake_ReceiptWriteFails(t *testing.T) {
	...
	eventPath := event.LogPath(soldierHome)
...
func TestDeliverWake_ObligationsInitFails(t *testing.T) {
	...
	receiptPath := turnend.ReceiptPath(captainHome, "test-obl-fail", "default")
	if _, err := os.Stat(receiptPath); err != nil {
		t.Errorf("receipt should exist even when obligations fail: %v", err)
	}

	// No ack should exist
	if turnend.IsReceiptAcked(captainHome, "test-obl-fail", "default") {
...
func TestDeliverWake_KeyDefault(t *testing.T) {
	...
	receiptPath := turnend.ReceiptPath(captainHome, "test-default-key", "default")
	if _, err := os.Stat(receiptPath); err != nil {
...
func runRelayTest(t *testing.T, req runReq) {
	...
	for i, tid := range req.taskIDs {
		...
		if err := turnend.WriteReceipt(captainHome, tid, tk, st, ""); err != nil {
			t.Fatalf("WriteReceipt %s: %v", tid, err)
		}
		if err := turnend.InitTaskObligations(captainHome, tid, tk); err != nil {
...
func TestReconcilePending_AlreadyAcked(t *testing.T) {
	...
	turnend.WriteAck(captainHome, taskID, termKey)
...
func TestReconcilePending_GeneralStatusWritten(t *testing.T) {
	...
	turnend.WriteReceipt(captainHome, taskID, termKey, "done", "completed")
	turnend.InitTaskObligations(captainHome, taskID, termKey)
...
// --- Context-aware safety fix: Pi agent prompt regression tests ---

// fakeActivationBackend implements Backend, AgentAwareBackend, and LegacyPrompt
// for testing the context-aware safety fix in ActivateOnReceipt.
type fakeActivationBackend struct {
	captureContent string
	captureErr     error
	agentAlive     bool
	paneExists     bool
	submitResult   session.PromptResult
	submittedText  string // captured for test assertions
}

func (f *fakeActivationBackend) NewWindow(_, _ string) (string, error) {
	return "", nil
}

func (f *fakeActivationBackend) SendKeys(_, _ string) error {
	return nil
}

func (f *fakeActivationBackend) Capture(_ string, _ int) (string, error) {
	return f.captureContent, f.captureErr
}

func (f *fakeActivationBackend) Alive(_ string) bool {
	return f.paneExists
}

func (f *fakeActivationBackend) Teardown(_ string) error {
	return nil
}

func (f *fakeActivationBackend) CheckAgentAlive(_ string) (bool, bool, error) {
	return f.paneExists, f.agentAlive, nil
}

func (f *fakeActivationBackend) LegacyPrompt(_ string, text string) session.PromptResult {
	f.submittedText = text
	return f.submitResult
}

// activationEnv creates a captain home with a parent home with captain meta
// and a receipt file, ready for ActivateOnReceipt testing.
func activationEnv(t *testing.T, bak *fakeActivationBackend) (captainHome, parentHome string) {
	t.Helper()
	captainHome = t.TempDir()
	parentHome = t.TempDir()
	...
	// Write captain provenance marker.
	initProvenance(t, captainHome, "test-captain")

	// Write captain task meta to parent home with herdr_pane_id.
	metaContent := "kind=captain\nherdr_pane_id=p1\nherdr_session=wsess\nbackend=herdr\n"
	...
	// Set up the test backend resolver.
	activationSessionResolver = func(_, _ string) (session.Backend, string, error) {
		return bak, "herdr", nil
	}
	t.Cleanup(func() { activationSessionResolver = nil })

	return captainHome, parentHome
}

// TestActivateOnReceipt_IdlePiAgent verifies that an idle pi agent showing ">"
// prompt IS treated as safe for injection when the backend confirms the target
// is a recognized agent. This is the primary regression test for the context-
// aware safety fix.
func TestActivateOnReceipt_IdlePiAgent(t *testing.T) {
	...
	// Write a receipt.
	if err := turnend.WriteReceipt(captainHome, "task-1", "key-1", "done", "complete"); err != nil {
		t.Fatalf("WriteReceipt: %v", err)
	}

	count := ActivateOnReceipt(captainHome, parentHome)
	if count != 1 {
		t.Errorf("expected 1 activation for idle pi agent, got %d", count)
	}

	// Activation-seen must be written.
	if !IsActivationSeen(captainHome, "task-1", "key-1") {
		t.Error("activation-seen should be written after successful submission")
	}

	// Verify the nudge was submitted.
	if !strings.Contains(bak.submittedText, "[receipt] done: soldier task-1 [key=key-1]") {
		t.Errorf("submitted text missing nudge content, got: %s", bak.submittedText)
	}
}

// TestActivateOnReceipt_IdlePiAgent_GreaterThan verifies both " > " and
// bare ">" (with variation in trailing whitespace) are accepted.
func TestActivateOnReceipt_IdlePiAgent_GreaterThanVariants(t *testing.T) {
	for name, content := range map[string]string{
		...
		t.Run(name, func(t *testing.T) {
			...
			if err := turnend.WriteReceipt(captainHome, "task-v", "key-v", "done", ""); err != nil {
				t.Fatalf("WriteReceipt: %v", err)
			}

			count := ActivateOnReceipt(captainHome, parentHome)
			if count != 1 {
				t.Errorf("expected 1 activation for %q content, got %d", name, count)
			}
			if !IsActivationSeen(captainHome, "task-v", "key-v") {
				t.Error("activation-seen should be written")
			}
		})
	}
}

// TestActivateOnReceipt_IdleClaudeAgent verifies that an idle claude agent
// showing "\u276F " (❯) is classified as safe even without the agent-aware
// override (the composer heuristic already classifies it correctly).
func TestActivateOnReceipt_IdleClaudeAgent(t *testing.T) {
	bak := &fakeActivationBackend{
		captureContent: "\u276F \n",
		agentAlive:     true,
		paneExists:     true,
	...
	if err := turnend.WriteReceipt(captainHome, "task-claude", "key-claude", "done", ""); err != nil {
		t.Fatalf("WriteReceipt: %v", err)
	}

	count := ActivateOnReceipt(captainHome, parentHome)
	if count != 1 {
		t.Errorf("expected 1 activation for idle claude agent, got %d", count)
	}
}
...
func TestActivateOnReceipt_BusyPiAgent(t *testing.T) {
	bak := &fakeActivationBackend{
	...
	if err := turnend.WriteReceipt(captainHome, "task-busy", "key-busy", "done", ""); err != nil {
		t.Fatalf("WriteReceipt: %v", err)
	}

	count := ActivateOnReceipt(captainHome, parentHome)
	if count != 0 {
		t.Errorf("expected 0 activations for busy agent, got %d", count)
	}

	// Must NOT mark activation-seen (preserve retry).
	if IsActivationSeen(captainHome, "task-busy", "key-busy") {
		t.Error("activation-seen should NOT be written for busy agent")
	}
}

// TestActivateOnReceipt_DeadShellNotSafe verifies that a bare ">" without
// a recognized agent is correctly NOT safe (existing behavior preserved).
func TestActivateOnReceipt_DeadShellNotSafe(t *testing.T) {
	bak := &fakeActivationBackend{
		captureContent: "> \n",
		agentAlive:     false, // no recognized agent
		paneExists:     true,
	}
	captainHome, parentHome := activationEnv(t, bak)

	if err := turnend.WriteReceipt(captainHome, "task-shell", "key-shell", "done", ""); err != nil {
		t.Fatalf("WriteReceipt: %v", err)
	}

	count := ActivateOnReceipt(captainHome, parentHome)
	if count != 0 {
		t.Errorf("expected 0 activations for dead shell, got %d", count)
	}

	if IsActivationSeen(captainHome, "task-shell", "key-shell") {
		t.Error("activation-seen should NOT be written for dead shell")
	}
}

// TestActivateOnReceipt_CaptureFailure preserves retry on capture errors.
func TestActivateOnReceipt_CaptureFailure(t *testing.T) {
	bak := &fakeActivationBackend{
		captureContent: "",
	...
	if err := turnend.WriteReceipt(captainHome, "task-cap", "key-cap", "done", ""); err != nil {
		t.Fatalf("WriteReceipt: %v", err)
	}

	count := ActivateOnReceipt(captainHome, parentHome)
	if count != 0 {
		t.Errorf("expected 0 activations on capture failure, got %d", count)
	}
	if IsActivationSeen(captainHome, "task-cap", "key-cap") {
		t.Error("activation-seen should NOT be written on capture error")
	}
}

// TestActivateOnReceipt_SubmitFailure preserves retry on submit failures.
func TestActivateOnReceipt_SubmitFailure(t *testing.T) {
	bak := &fakeActivationBackend{
		captureContent: "> \n",
		agentAlive:     true,
		paneExists:     true,
		submitResult: session.PromptResult{
			Status: session.PromptStalled,
			Detail: "agent stalled",
		},
	}
	captainHome, parentHome := activationEnv(t, bak)

	if err := turnend.WriteReceipt(captainHome, "task-sub", "key-sub", "done", ""); err != nil {
		t.Fatalf("WriteReceipt: %v", err)
	}

	count := ActivateOnReceipt(captainHome, parentHome)
	if count != 0 {
		t.Errorf("expected 0 activations on submit failure, got %d", count)
	}
	if IsActivationSeen(captainHome, "task-sub", "key-sub") {
		t.Error("activation-seen should NOT be written after submit failure")
	}
}

// TestActivateOnReceipt_PendingContentNotSafe verifies that a pi agent with
// pending typed content (e.g., "\u276F git push") is NOT safe even with
// agent registration.
func TestActivateOnReceipt_PendingContentNotSafe(t *testing.T) {
	bak := &fakeActivationBackend{
		captureContent: "\u276F git push\n",
		agentAlive:     true,
		paneExists:     true,
	}
	captainHome, parentHome := activationEnv(t, bak)

	if err := turnend.WriteReceipt(captainHome, "task-pend", "key-pend", "done", ""); err != nil {
		t.Fatalf("WriteReceipt: %v", err)
	}

	count := ActivateOnReceipt(captainHome, parentHome)
	if count != 0 {
		t.Errorf("expected 0 activations for pending content, got %d", count)
	}
	if IsActivationSeen(captainHome, "task-pend", "key-pend") {
		t.Error("activation-seen should NOT be written with pending content")
	}
}
...
func TestActivateOnReceipt_AlreadySeenIdempotent(t *testing.T) {
	...
	// Write receipt and mark as already seen.
	if err := turnend.WriteReceipt(captainHome, "task-seen", "key-seen", "done", ""); err != nil {
		t.Fatalf("WriteReceipt: %v", err)
	}
	if err := MarkActivationSeen(captainHome, "task-seen", "key-seen"); err != nil {
		t.Fatalf("MarkActivationSeen: %v", err)
	}
...
// TestActivateOnReceipt_DiagnosticsLogging verifies that diagnostic entries
// are printed to stderr with the expected prefix.
func TestActivateOnReceipt_DiagnosticsLogging(t *testing.T) {
	// Test with two receipts: one that succeeds and one already seen.
	bak := &fakeActivationBackend{
		captureContent: "> \n",
		agentAlive:     true,
		paneExists:     true,
	...
	// First receipt: new, should succeed.
	if err := turnend.WriteReceipt(captainHome, "diag-new", "diag-new-key", "done", "ok"); err != nil {
		t.Fatalf("WriteReceipt: %v", err)
	}

	// Second receipt: mark as already seen to test skip diagnostic.
	if err := turnend.WriteReceipt(captainHome, "diag-seen", "diag-seen-key", "failed", "nope"); err != nil {
		t.Fatalf("WriteReceipt: %v", err)
	}
	if err := MarkActivationSeen(captainHome, "diag-seen", "diag-seen-key"); err != nil {
		t.Fatalf("MarkActivationSeen: %v", err)
	}

	// Run ActivateOnReceipt — diagnostics are logged to stderr.
	count := ActivateOnReceipt(captainHome, parentHome)
	if count != 1 {
...
// TestCheckAgentComposerSafe verifies the helper function directly.
func TestCheckAgentComposerSafe(t *testing.T) {
	tests := []struct {
		name    string
		content string
		want    bool
	}{
		{"pi-prompt-trailing-space", "> \n", true},
		{"pi-prompt-no-space", ">\n", true},
		{"claude-prompt", "\u276F \n", true},
		{"codex-prompt", "\u203A \n", true},
		{"empty-capture", "\n", true},
		{"multi-line-pi", "some output\n> \n", true},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			fake := &fakeActivationBackend{
				captureContent: tt.content,
				paneExists:     true,
			}
			got := checkAgentComposerSafe(fake, "session:test")
			if got != tt.want {
				t.Errorf("checkAgentComposerSafe = %v, want %v (content=%q)", got, tt.want, tt.content)
			}
		})
	}
}
