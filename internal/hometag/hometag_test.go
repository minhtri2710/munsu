...
func TestWorkspaceTagSecondmateIsReadableAndScoped(t *testing.T) {
	homeDir := t.TempDir()
	marker := "munsu-v2\nAPI Supervisor\n" + homeDir + "\n"
	if err := os.WriteFile(filepath.Join(homeDir, ".munsu-secondmate-home"), []byte(marker), 0644); err != nil {
		t.Fatal(err)
	}
	got := WorkspaceTag(homeDir)
	wantPrefix := "2ndmate-api-supervisor-"
...
func TestWorkspaceTagIgnoresMalformedSecondmateMarker(t *testing.T) {
	homeDir := t.TempDir()
	if err := os.WriteFile(filepath.Join(homeDir, ".munsu-secondmate-home"), []byte("broken\n"), 0644); err != nil {
		t.Fatal(err)
	}
	if got, want := WorkspaceTag(homeDir), Tag(homeDir); got != want {
...
func TestWorkspaceTagCollapsesTmpAliases(t *testing.T) {
	...
	marker := "munsu-v2\napi-sup\n" + realHome + "\n"
	if err := os.WriteFile(filepath.Join(realHome, ".munsu-secondmate-home"), []byte(marker), 0644); err != nil {
		t.Fatal(err)
	}
