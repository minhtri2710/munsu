...
// callers read false as permission to act -- ClaimWatcherLease reclaims the
// lease, which makes the lease layer stop being a singleton guard while still
// reading like one, and IsWatcherLeaseHealthy declares the lease unhealthy --
...
// (watcher_lease.go), so a drifting signature turns the lane red there before it
// reaches any test file. Do not read that test as coverage for the behaviour
// above.
