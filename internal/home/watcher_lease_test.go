...
func TestWatcherLease_Observe(t *testing.T) {
	home := t.TempDir()
	os.MkdirAll(filepath.Join(home, "state"), 0755)

	// Observe empty home — absent.
	summary := ObserveWatcherLease(home)
	if summary == nil {
		t.Fatal("ObserveWatcherLease should not return nil")
	}
	if !summary.Absent {
		t.Error("expected absent for empty home")
	}

	// Claim lease and write beat.
	pid := os.Getpid()
	ClaimWatcherLease(home, pid)
	WriteWatcherBeat(home)

	// Observe — should be healthy.
	summary = ObserveWatcherLease(home)
	if summary == nil {
		t.Fatal("ObserveWatcherLease should not return nil")
	}
	if summary.Absent {
		t.Error("expected not absent")
	}
	if !summary.Healthy {
		t.Error("expected healthy")
	}
	if summary.PID != pid {
		t.Errorf("PID = %d, want %d", summary.PID, pid)
	}
	if summary.Age == "" {
...
func TestWatcherLease_ObserveStale(t *testing.T) {
	home := t.TempDir()
	os.MkdirAll(filepath.Join(home, "state"), 0755)
	...
	if summary.Healthy {
		t.Error("expected unhealthy for stale beat")
	}
	if !summary.Stale {
...
func TestWatcherLease_ObserveDeadPID(t *testing.T) {
	home := t.TempDir()
	os.MkdirAll(filepath.Join(home, "state"), 0755)

	// Claim lease with a PID that's definitely not alive.
	...
	if summary.Healthy {
		t.Error("expected unhealthy for dead PID")
	}
}
