...
func TestRunCycleWithProbeAndSenderRejectsNilHooksWithoutMarkingRecoveryDone(t *testing.T) {
	...
	if _, err := RunCycleWithProbeAndSender(home, testEndpointProbe{}, &testMailboxSender{}, nil, NoopRetirementPort{}, testTaskStatePort{}); err == nil {
...
func TestRunCycleWithProbeAndSenderRejectsNilSenderWithoutMarkingRecoveryDone(t *testing.T) {
	home := t.TempDir()
	recoveryDone.Delete(home)
	if _, err := RunCycleWithProbeAndSender(home, testEndpointProbe{}, nil, NoopWatcherHooks{}, NoopRetirementPort{}, testTaskStatePort{}); err == nil {
		t.Fatal("expected missing sender error")
	}
	if _, loaded := recoveryDone.Load(home); loaded {
		t.Fatal("nil sender marked recovery complete")
	}
	if _, err := RunCycleWithProbeAndSender(home, testEndpointProbe{}, &testMailboxSender{}, NoopWatcherHooks{}, NoopRetirementPort{}, testTaskStatePort{}); err != nil {
...
func TestRunCycleRecoveryUsesExplicitSenderAndWritesMarker(t *testing.T) {
	...
	if _, err := RunCycleWithProbeAndSender(home, testEndpointProbe{}, sender, NoopWatcherHooks{}, NoopRetirementPort{}, testTaskStatePort{}); err != nil {
		t.Fatal(err)
	}
