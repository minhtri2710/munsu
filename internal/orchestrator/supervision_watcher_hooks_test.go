...
func TestWatcherHooksRunStartupOnceThenCycleAndActivation(t *testing.T) {
	...
	if _, err := RunCycleWithProbeAndSender(home, testEndpointProbe{}, &testMailboxSender{}, hooks, NoopRetirementPort{}, acceptingCheckValidationPort{}, testTaskStatePort{}); err != nil {
		t.Fatal(err)
	}
	if _, err := RunCycleWithProbeAndSender(home, testEndpointProbe{}, &testMailboxSender{}, hooks, NoopRetirementPort{}, acceptingCheckValidationPort{}, testTaskStatePort{}); err != nil {
		t.Fatal(err)
...
func TestWatcherHooksAreIsolatedPerHomeAndInstance(t *testing.T) {
	...
	if _, err := RunCycleWithProbeAndSender(homeA, testEndpointProbe{}, &testMailboxSender{}, hooksA, NoopRetirementPort{}, acceptingCheckValidationPort{}, testTaskStatePort{}); err != nil {
		t.Fatal(err)
	}
	if _, err := RunCycleWithProbeAndSender(homeB, testEndpointProbe{}, &testMailboxSender{}, hooksB, NoopRetirementPort{}, acceptingCheckValidationPort{}, testTaskStatePort{}); err != nil {
		t.Fatal(err)
	}
	if _, err := RunCycleWithProbeAndSender(homeA, testEndpointProbe{}, &testMailboxSender{}, hooksA, NoopRetirementPort{}, acceptingCheckValidationPort{}, testTaskStatePort{}); err != nil {
		t.Fatal(err)
