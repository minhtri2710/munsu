...
4. Report status by appending one line:
   ...
   munsu reads your pane for that.
   Use `+"`"+`paused: {why}`+"`"+` - distinct from `+"`"+`blocked:`+"`"+` - ONLY when you are deliberately idling on a
   known external wait you expect to clear on its own (an upstream release, a rate-limit reset,
   a scheduled window): munsu then leaves your idle pane alone and rechecks it on a long
   cadence instead of treating it as a possible wedge. Use `+"`"+`blocked:`+"`"+` when you are stuck and need help.
5. If you hit the same obstacle twice, append `+"`"+`blocked: {why}`+"`"+` and stop; munsu will help.
6. If a decision belongs to a human (product choices, destructive actions, ask-user findings),
   append `+"`"+`needs-decision: {summary of options}`+"`"+` and stop. munsu will reply with the decision.
   When munsu replies or a blocker clears and you resume, append `+"`"+`resolved: {how it was decided or unblocked}`+"`"+` (add the same `+"`"+`[key=<slug>]`+"`"+` if you opened it with one) so the decision or blocker is durably closed and does not keep resurfacing.
7. Never stop, restart, or update the shared `+"`"+`no-mistakes`+"`"+` daemon - it is one instance serving
   every lane/home, so restarting it kills other lanes' in-flight pipeline runs. On ANY no-mistakes
   daemon error, append `+"`"+`blocked: {the daemon error}`+"`"+` and stop; only munsu manages the daemon.
...
munsu will then instruct you to run /no-mistakes to validate and ship a PR.

You drive no-mistakes by responding to its gates, not by implementing fixes.
Follow the guidance no-mistakes itself provides for the mechanics: it loads when you invoke /no-mistakes, and `+"`"+`no-mistakes axi run --help`+"`"+` plus the `+"`"+`help`+"`"+` lines in each `+"`"+`axi`+"`"+` response are authoritative and version-matched to the installed binary.
Do not hand-edit, commit, or fix findings yourself while a run is active - the pipeline applies every fix.

Two munsu-specific rules layer on top of that guidance:
- ask-user findings are not yours to answer: escalate to munsu (rule 6) and stop.
  When the decision comes back, feed it to the gate with `+"`"+`no-mistakes axi respond`+"`"+` and let the pipeline apply it - do not route the question to "the user" or implement the fix yourself.
- Avoid `+"`"+`--yes`+"`"+`: munsu, not you, owns the ask-user decisions it would silently auto-resolve.
