...
the `Flake ledger` workflow goes red with the exact diff on its run summary. It
prepares no branch for you: GitHub Actions is not permitted to open pull requests
in this repository, so the lane stops at reporting and a person applies it by
running `.github/scripts/flake-sweep.sh sync` locally. It is not a sampler
either: it never re-runs anything and it computes no failure rate. Four pushes
to `main` on 2026-08-14 13:40-13:41
