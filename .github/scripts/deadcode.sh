...
# reprinted on every run. These are real holes -- a guard that exists, is
# complete, and cannot fire -- listed only so the lane can go green; each names
# the issue that owns the verdict. An annotation on every single run is the
# difference between a waiver and a silent line.
announce_open_bugs() {
	awk -F '\t' '
		/^[[:space:]]*#/ || /^[[:space:]]*$/ { next }
		$3 ~ /^OPEN-BUG/ {
